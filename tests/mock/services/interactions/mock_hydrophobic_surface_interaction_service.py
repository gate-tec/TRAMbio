...
class MockEmptyHydrophobicSurfaceInteractionService(IInteractionService):
    """
...
class MockHydrophobicSurfaceInteractionService(IInteractionService):
    """
    Parameter-independent service ensuring no interactions.
    """
    ...
    def apply_interactions(self, protein_graph: ProteinGraph, parameter_id: str, verbose: bool = False):
        ...
        pivots = pdb_df\
            .loc[pdb_df.element_symbol == "C", ["node_id", "atom_name", "element_symbol"]]\
        ...
        for _, row in pivots.iterrows():
            ...
            for i in range(len(node_ids) - 1):
                for j in range(i + 1, len(node_ids)):
                    interactions.append({
                        'node_1': node_ids[i], 'node_2': node_ids[j], 'bond_length': 5.0, 'bond_type': InteractionType.HYDROPHOBIC.value
                    })
